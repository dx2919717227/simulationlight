...
class DQN:
    def __init__(self):
        ...
        self.model.load_weights("E:\\vscodeproject\\Python\\simlight\\dqn_cor_10.h5")
    ...
    def train(self, episode, dis):
        ...
        for i in range(episode):
            ...
            while not done:
                ...
                cloud_output = self.cloud_model.predict(x)[0]
                x_new = np.append(x[0], cloud_output).reshape(-1, 6)
                action, choice, q_values = self.egreedy_action(x_new)
                observation, reward, done, info = self.env.step(dis, action)

                lux = observation[1]
                observation = self.process_observation(observation)
                cloud_output_next = self.cloud_model.predict(observation)[0]
                observation_new = np.append(observation[0], cloud_output_next).reshape(-1, 6)
                ...
                reward_sum += reward
                self.remember(x_new[0], action, reward, observation_new[0], done)
        ...
        #if dis == 'dis10':
        #self.model.save_weights('dqn_cor_10.h5')
        #print(u'当前进程的内存使用：%.4f GB' % (psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024) )
        self.plot()
        # return history
